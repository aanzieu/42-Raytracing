		...
		int i, t_hyperboloid *hyper)
{
	if (ft_strnequ(tmp[i], "<top>", ft_strlen("<top>")))
		...
		parse_reflection(&hyper->reflection_coef, tmp, world->line);
	else if (ft_strnequ(tmp[i], "<refraction>", ft_strlen("<refraction>")))
		parse_refraction(&hyper->refraction_coef, tmp, world->line);
...
void					parse_hyperboloid(t_world *world, t_list *lst)
{
	...
	add_hyperboloid(&world->hyperboloids_tmp, new_hyperboloid(hyper, world->id++));
	free(hyper);
}
