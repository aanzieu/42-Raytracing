...
void load_video_buffer(int *a_h, unsigned char *video_buffer, int x_res, int render_factor)
{
	int i;
	int j;
	int x;
...
struct nk_image screen_load(int *a_h, unsigned char *video_buffer, int x_res, int render_factor)
{
    int     x;
    int     y;
    GLuint  tex;
                            ...
                            t_world *w)
{
        ...
        | NK_WINDOW_CLOSABLE))
    {
        ...
        {
            screen = screen_load(w->a_h, w->video_buffer, w->viewplane.x_res, w->render_factor);
            w->reload_buffer = 0;
        }
