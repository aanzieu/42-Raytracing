...
static void draw_p_presets_next(struct nk_context *c, t_world *w, int i)
{
	...
	option = get_preset_info(w->planes[i].perlin);
	if (nk_button_symbol_label(c, (option == 2) ?
	NK_SYMBOL_CIRCLE_SOLID:NK_SYMBOL_CIRCLE_OUTLINE, "MARBLE", NK_TEXT_LEFT))
	{
		option = 2;
		if (w->planes[i].perlin.is_set == 1)
			w->planes[i].perlin.is_set = 0;
		w->planes[i].perlin.pre_set = MARBLE;
		w->planes[i].perlin.scale = 0.9;
		w->redraw = 1;
	}
	...
	NK_SYMBOL_CIRCLE_SOLID:NK_SYMBOL_CIRCLE_OUTLINE, "GLASS", NK_TEXT_LEFT))
	{
		...
		if (w->planes[i].perlin.is_set == 1)
			w->planes[i].perlin.is_set = 0;
		w->planes[i].perlin.pre_set = GLASS;
	}
}
	...
	t_world *w, int i)
{
		...
		NK_SYMBOL_CIRCLE_SOLID:NK_SYMBOL_CIRCLE_OUTLINE, "WOOD", NK_TEXT_LEFT))
	{
		...
		if (w->planes[i].perlin.is_set == 1)
			w->planes[i].perlin.is_set = 0;
		w->planes[i].perlin.pre_set = WOOD;
		w->planes[i].perlin.scale = 0.19;
		w->redraw = 1;
	}
...
static void draw_infos_next(struct nk_context *c, struct media *m, t_world *w,\
	int i)
{
	if (ui_widget_value_infos(c, m, &w->planes[i].perlin.scale,\
			"PERLIN NOISE SCALE:"))
	{
		if (w->planes[i].perlin.is_set == 0 && w->planes[i].perlin.scale > 0)
			w->planes[i].perlin.is_set = 1;
		if (w->planes[i].perlin.scale > 0 && w->planes[i].perlin.amount > 0)
			w->redraw = 1;
	}
	else if (ui_widget_value_infos(c, m, &w->planes[i].perlin.amount,\
			"PERLIN NOISE AMOUNT:"))
	{
		if (w->planes[i].perlin.is_set == 0 && w->planes[i].perlin.amount > 0)
			w->planes[i].perlin.is_set = 1;
		if (w->planes[i].perlin.scale > 0 && w->planes[i].perlin.amount > 0)
			w->redraw = 1;
	}
...
static void	draw_infos(struct nk_context *c, struct media *m, t_world *w,\
	int i)
{
	if (ui_widget_value_infos(c, m, &w->planes[i].pos.x, "POS X:"))
		w->redraw = 1;
	else if (ui_widget_value_infos(c, m, &w->planes[i].pos.y, "POS Y:"))
		w->redraw = 1;
	else if (ui_widget_value_infos(c, m, &w->planes[i].pos.z, "POS Z:"))
		w->redraw = 1;
	else if (ui_widget_value_infos(c, m, &w->planes[i].up.x, "NORMAL X:"))
		w->redraw = 1;
	else if (ui_widget_value_infos(c, m, &w->planes[i].up.y, "NORMAL Y:"))
		w->redraw = 1;
	else if (ui_widget_value_infos(c, m, &w->planes[i].up.z, "NORMAL Z:"))
		w->redraw = 1;
	else if (ui_widget_value_infos(c, m, &w->planes[i].reflection_coef,\
			"REFLECTION:"))
		w->redraw = 1;
	else if (ui_widget_value_infos(c, m, &w->planes[i].refraction_coef,\
			"REFRACTION:"))
		w->redraw = 1;
	else if (ui_widget_value_infos(c, m, &w->planes[i].transparence_coef,\
			"TRANSPARENCE:"))
		w->redraw = 1;
	else
