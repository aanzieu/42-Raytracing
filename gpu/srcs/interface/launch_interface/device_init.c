...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/10/02 12:49:34 by aanzieu           #+#    #+#             */
/*   Updated: 2017/10/02 14:16:31 by aanzieu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			buffer_setup_end_next(struct device *dev)
{
...
static void			buffer_setup_end(struct device *dev)
{
	...
	vs = sizeof(struct nk_glfw_vertex);
	buffer_setup_end_next(dev);
	glVertexAttribPointer((GLuint)dev->attrib_pos,
			2, GL_FLOAT, GL_FALSE, vs, (void*)vp);
	glVertexAttribPointer((GLuint)dev->attrib_uv,
			2, GL_FLOAT, GL_FALSE, vs, (void*)vt);
	glVertexAttribPointer((GLuint)dev->attrib_col,
			4, GL_UNSIGNED_BYTE, GL_TRUE, vs, (void*)vc);
...
void				device_init(struct device *dev)
{
	...
	fragment_shader = NK_SHADER_VERSION	"precision mediump float;\n"
