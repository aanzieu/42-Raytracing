...
extern "C" void render_cuda(int *a_h, unsigned int constw, unsigned int consth, t_world world, int reset)
{
	...
	// cudaMalloc(&h_cubes_d, sizeof(t_h_cube) * (1 + world.h_cubes_len));
	// cudaMalloc(&triangles_d, sizeof(t_triangle) * (1 + world.triangles_len));
	// cudaMalloc(&torus_d, sizeof(t_torus) * (1 + world.torus_len));
	// cudaMalloc(&mobius_d, sizeof(t_mobius) * (1 + world.mobius_len));
	// cudaMalloc(&paraboloids_d, sizeof(t_paraboloid) * (1 + world.paraboloids_len));
